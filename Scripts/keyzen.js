...
function repeatCombs(combs) {
    let res = [];
    for (let i = 0; i < combs.length; i++) {
        res.push(combs[i]);
        res.push(combs[i].split("").reverse().join(""));
    }
    return res;
}
...
function makeCombinations() {
    ...
    res = res.flat();
    res = repeatCombs(res);
    res.unshift(oneLetterCombinations(mainChar));
...
function setLevel(level) {
    ...
    data.level_chars = get_level_chars();
    data.counter = 0;
    data.combinations = makeCombinations();
}
...
function render_level_bar() {
    ...
    m = Math.floor($('#level-chars-wrap').innerWidth() * Math.min(1.0, 1));
...
function generate_word() {
    ...
    if (data.level > 1 && Object.keys(data.word_errors).length !== 0) {
        console.log("yes")
        data.counter--;
    }
    if (data.counter >= data.combinations.length) {
        setLevel(data.level + 1);
    }    
    data.word_errors = {};
    return data.combinations[data.counter++];
}
